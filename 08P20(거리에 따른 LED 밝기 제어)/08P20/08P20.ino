...
float dist_min, dist_max, dist_raw, dist_raw_save; // unit: mm
...
// turn on the LED if the distance is between dist_min and dist_max
  ...
  if(dist_raw >= 100 && dist_raw <= 200){
    value = (-255.000/100.000)*dist_raw+510;
  }
  else{
    value = (255.000/100.000)*dist_raw-510;
  }
...
{
  float reading, reading_save;
  digitalWrite(TRIG, HIGH);
  delayMicroseconds(10);
  digitalWrite(TRIG, LOW);
  
  reading = pulseIn(ECHO, HIGH, timeout) * scale; // unit: mm
  if(reading < dist_min || reading > dist_max) reading = reading_save; // return 0 when out of range.
  else reading_save = reading;
  return reading;
